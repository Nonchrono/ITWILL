...
<%
	...
	out.print(request.getParameter("uname"));60
	out.print("<hr>");
	...
	request 객체가 가지고 있는 값을 제대로 가져올 수 없다
	...
	*/
	...
	try {
		...
		// 열거형 Enumeration enum = {"KIM", "LEE", "PARK"} 자료형 지정이 필요가 없다
		...
		out.print("renamed 파일명 : " + fileName);
		out.print("<hr>");
		
		
		// 4) /storage 폴더에 저장된 파일의 기타 정보 확인하기
		File file = mr.getFile(item); // 파일을 담고 있는 name값(item)에 해당하는 실제 파일 가져오기
