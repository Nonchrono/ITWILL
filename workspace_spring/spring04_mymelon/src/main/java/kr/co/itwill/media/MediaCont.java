...
public class MediaCont {
    ...
    public ModelAndView createProc(@ModelAttribute MediaDTO dto, HttpServletRequest req) {
        // 매개변수로 String title, MultipartFile posterMF, MultipartFile filenameMF를 사용할 수도 있다.
        ...
        dto.setFilename(filename); // 리네임된 파일명을 dto 객체에 담기
        dto.setFilesize(filenameMF.getSize()); // 파일 사이즈를 dto 객체에 담기
        ...
        if(cnt == 0) {
            ...
            String img = "<img src='../images/fail.png' width: 100px;>";
            String link1 = "<input type='button' value='다시시도' onclick='javascript:history.back()'>";
            String link2 = "<input type='button' value='목록' onclick='location.href=\"list.do?mediagroupno=" + dto.getMediagroupno() + "\"'>";
            ...
            String img = "<img src='../images/success.png' width: 100px;>";
            String link2 = "<input type='button' value='목록' onclick='location.href=\"list.do?mediagroupno=" + dto.getMediagroupno() + "\"'>";
            mav.addObject("msg1", msg1);
            mav.addObject("img", img);
            mav.addObject("link2", link2);
    ...
    public ModelAndView deleteProc(int mediano, HttpServletRequest req) {
        ...
        if (cnt == 0) {
            ...
            String img = "<img src='../images/fail.png' width: 100px;>";
            String link1 = "<input type='button' value='다시시도' onclick='javascript:history.back()'>";
            String link2 = "<input type='button' value='목록' onclick='location.href=\"list.do?mediagroupno=" + oldDTO.getMediagroupno() + "\"'>";
            mav.addObject("msg1", msg1);
            mav.addObject("img", img);
            mav.addObject("link1", link1);
            mav.addObject("link2", link2);
            ...
            String img = "<img src='../images/success.png' width: 100px;>";
            String link1 = "<input type='button' value='목록' onclick='location.href=\"list.do?mediagroupno=" + oldDTO.getMediagroupno() + "\"'>";
            mav.addObject("msg1", msg1);
            mav.addObject("img", img);
            mav.addObject("link1", link1);
    ...
    public ModelAndView updateProc(@ModelAttribute MediaDTO dto, HttpServletRequest req) {
        ...
        if (posterMF.getSize() > 0) { // 새로운 포스터 파일이 첨부되어 전송되었는지? 0보다 작으면 안 보냈다는 뜻
        ...
        if (posterMF.getSize() > 0) { // 새로운 포스터 파일이 첨부되어 전송되었는지? 0보다 작으면 안 보냈다는 뜻
            ...
            // 포스터 파일은 수정하지 않은 경우
            dto.setPoster(oldDTO.getFilename()); // 기존에 저장된 파일명
