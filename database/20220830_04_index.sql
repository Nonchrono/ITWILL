...
-- where절로 걸러낼 내용이 많은 컬럼에서는 index 페이지로 만들어놓는 것이 속도면에서 좋다.
-- 70 ~ 80% 실제 메모리, 20 ~ 30% 인덱스 등에 사용
...
drop index emp2_name_idx; -- 인덱스 삭제
...
select * from emp4 where no = 466;            -- Full scan, COST 891

select * from emp4 where sal > 300;           -- Full scan, COST 892
...
create index emp4_name_idx on emp4(name, sal);
